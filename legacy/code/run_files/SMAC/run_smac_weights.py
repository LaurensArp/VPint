...
bbox_taichung_tr = (120.9129,24.3997) # 120.8129
...
MRP_iter_smac = UniformIntegerHyperparameter("MRP_iter", 1, 500, default_value=100)
optimisation_epochs_smac = UniformIntegerHyperparameter("optimisation_epochs", 1, 500, default_value=100)
mutation_intensity_smac = UniformFloatHyperparameter("mutation_intensity", 0.0001, 20, default_value=0.5)
train_proportion_smac = UniformFloatHyperparameter("train_proportion", 0.01, 1, default_value=0.8)

cs.add_hyperparameters([MRP_iter_smac,optimisation_epochs_smac,mutation_intensity_smac,train_proportion_smac])
...
type_top_n_variable_smac = UniformIntegerHyperparameter("type_top_n_variable", 1, 50, default_value=15)
...
max_runtime_seconds = max_runtime * 60 * 60
...
with open("/anonymised/path/smac_output/conditions.csv",'a') as fp:
    s = str(smac_current_id) + "," + label_set + ",weights," + training_set_name + "," + test_set_name + "\n"
        

scenario = Scenario({"run_obj": "quality",
                     #"runcount-limit": 50,
                     "memory_limit": max_memory_usage_mb,
                     "output_dir": "/anonymised/path/smac_output/" + label_set + "/weights/" + str(smac_current_id),
...
def run_config_weights(cfg):
    ...
    # Supervise by optimisation code
    
    # Train model

    MRP = normalise_attributes(super_H_train,cfg["feature_normalisation_method"])
    model, pred_loss, opt_loss = supervise_by_optimisation_auto(MRP,cfg["MRP_iter"],cfg["optimisation_epochs"],
                                                      cfg["mutation_intensity"],cfg["train_proportion"],
                                                                autoskl_params_weights,average_loss)

    # Run MRP with predicted weights on training set
    #MRP = normalise_attributes(super_H_train,cfg["feature_normalisation_method"])
    #MRP_final_train,mae_train,pred_train = run_MRP_with_model(MRP,model,cfg["MRP_iter"])

    # Run on test set

    MRP = normalise_attributes(super_H_test,cfg["feature_normalisation_method"])
    MRP_final_test,mae_test,pred_test = run_MRP_with_model(MRP,model,cfg["MRP_iter"])
...
smac = SMAC4AC(scenario=scenario, tae_runner=run_config_weights)
